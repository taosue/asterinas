...
- **Multiple SCML files support**: Specify a directory to automatically load all `.scml` files from the directory and its
subdirectories. Each file maintains its own scope for bitflags and struct definitions, preventing cross-file pollution.
...
```bash
sctrace <SCML_PATH> [OPTIONS] -- [program] [args...]
```
...
sctrace ./scml_patterns/ --input trace.log --quiet
```
...
sctrace ./syscall-feature-coverage/ -- tree .

# Monitor process information calls
sctrace ./syscall-feature-coverage/ -- top

# Monitor network operations
sctrace ./syscall-feature-coverage/ -- ping 127.0.0.1
```
