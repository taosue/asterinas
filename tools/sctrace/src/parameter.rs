...
pub struct Parameters {
    /// Path to the SCML pattern file or directory
    scml_path: String,
...
impl Parameters {
    ...
    ///
    /// ```text
    /// sctrace <SCML_PATH> [--input FILE] [--quiet] [-- PROGRAM] [PROGRAM_ARGS...]
    /// ```
    pub fn new(args: Vec<String>) -> Result<Self, ParameterError> {
        let app = Command::new("sctrace")
            ...
            .arg(
                Arg::new("scml_path")
                    .help("Path to SCML file or directory")
                    .required(true)
                    .index(1),
            )
            .arg(
        ...
        let scml_path = matches.get_one::<String>("scml_path").unwrap().clone();
