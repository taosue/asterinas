...
impl<'a> Patterns<'a> {
    ...
    /// If path is a directory, reads all .scml files in it.
    pub fn from_scml_file(path: &str) -> Result<Self, ScmlParseError> {
        let path_obj = Path::new(path);

        let content = if path_obj.is_dir() {
            // Recursively read all .scml files from directory and subdirectories
            let mut combined_content = String::new();
            Self::read_scml_files_recursive(path_obj, &mut combined_content)?;

            if combined_content.is_empty() {
                return Err(ScmlParseError::IoError(format!(
                    "No .scml files found in directory '{}' or its subdirectories",
                    path
                )));
            }
    ...
    /// Recursively reads all .scml files from a directory and its subdirectories.
    fn read_scml_files_recursive(
        dir: &Path,
        combined_content: &mut String,
    ) -> Result<(), ScmlParseError> {
        ...
        for entry in entries {
            let entry = entry.map_err(|e| {
                ScmlParseError::IoError(format!("Failed to read directory entry: {}", e))
            })?;
            ...
            if file_path.is_dir() {
                ...
                Self::read_scml_files_recursive(&file_path, combined_content)?;
            } else if file_path.is_file()
                && file_path.extension().and_then(|s| s.to_str()) == Some("scml")
            {
                // Read .scml file
