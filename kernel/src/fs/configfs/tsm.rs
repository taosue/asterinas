...
use ostd::{
    mm::{VmReader, VmWriter, FallibleVmRead, FallibleVmWrite},
    sync::RwLock,
};
use spin::Once;

const TSM_INBLOB_MAX: usize = 64;
const TSM_OUTBLOB_MAX: usize = 32 * 1024;
    ...
    fn read_attr_at(&self, name: &str, offset: usize, writer: &mut VmWriter) -> Result<usize> {
        match name {
            ...
            "outblob" => {
                ...
                if offset == 0 {
                    // Get quote
                    data.outblob = match tdx_get_quote(&data.inblob) {
                        Ok(quote) => Some(quote),
                        Err(_) => return Err(Error::AttributeError),
                    };
                }

                if data.outblob.is_none() {
                    return Err(Error::AttributeError);
                }

                if offset >= data.outblob.as_ref().unwrap().len() {
                    return Ok(0);
                }

                let outblob_ref = data.outblob.as_ref().unwrap();
                let slice = &outblob_ref[offset..];
