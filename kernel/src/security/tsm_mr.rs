...
pub(super) fn init() {
    let node = {
        ...
        // FIXME: Temporary folder node until we have a proper sysfs class implementation
        let misc_node = FolderNode::new("misc");
        misc_node.add_child(tdx_guest_node).unwrap();
        let class_node = FolderNode::new("class");
        class_node.add_child(misc_node).unwrap();
...
/// A systree node representing the `/sys/class/misc/tdx_guest` directory.
///
/// This node serves as the root for all TDX guest-related sysfs entries,
/// including TDX guest parameters, debugging interfaces, and various
/// TDX guest subsystem information. It corresponds to the `/tdx_guest`
...
impl TdxGuestSysNodeRoot {
    /// Creates a new `TdxGuestSysNodeRoot` instance.
    fn new() -> Arc<Self> {
        let name = SysStr::from("tdx_guest");
        let builder = SysAttrSetBuilder::new();
        let attrs = builder
            .build()
            .expect("Failed to build TDX guest attribute set");
    ...
    /// Adds a TDX guest `SysNode` to this node.
...
inherit_sys_leaf_node!(Measurement, fields, {
    ...
    fn read_attr_at(&self, name: &str, offset: usize, writer: &mut VmWriter) -> Result<usize> {
        ...
        if attr.refresh_on_read() && *in_sync == false {
            let mut in_sync_write = in_sync.upgrade();
            if *in_sync_write == false {
