...
pub fn tdx_get_quote(inblob: &[u8]) -> Result<Box<[u8]>> {
    ...
    entry.buf.write_bytes(offset_of!(tdx_quote_hdr, version), &1u64.to_le_bytes())?;
    entry.buf.write_bytes(offset_of!(tdx_quote_hdr, status), &0u64.to_le_bytes())?;
    entry.buf.write_bytes(offset_of!(tdx_quote_hdr, in_len), &(TDX_REPORT_LEN as u32).to_le_bytes())?;
    entry.buf.write_bytes(offset_of!(tdx_quote_hdr, out_len), &0u32.to_le_bytes())?;
    entry.buf.write_bytes(offset_of!(tdx_quote_hdr, data), &report)?;
    ...
    entry.buf.read_bytes(size_of::<tdx_quote_hdr>(), outblob.as_mut())?;
    Ok(outblob)
}
